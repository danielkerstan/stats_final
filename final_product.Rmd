...
```{r, fig.cap= "Spatial interpolation of average yearly precipitation in Santa Barbara County"}
...
```{r, fig.cap= "Spatially interpolated yearly precipitation in Santa Barbara County"}
...
With the precipitation data sorted out I can now apply this to understanding to the effect precipitation on the water level of the Cachuma Reservoir.
The Cachuma Reservoir is heavily relied on by the city of Santa Barbara, which is entitled to 32.19% of its available water. Understanding how rainfall affects this reservoir is vitally important.
...
```{r, fig.cap= "Correlation between precipitation and reservoir level change based on r squared value attained through linear regression analysis"}
...
According to this model, with no rain there is  a monthly change in reservoir level of `r round(model$coefficients[1], 3)` feet and there is a monthly change of `r round(model$coefficients[2], 3` ft for every inch of rain in a month.
